    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.html') and file != 'index-2.html':
                ...
                # Robust pattern to find the old card block
                # Looking for the col-lg-3 that wraps vjs-glance-card-v3 or similar
                # We'll use a very broad match for the inner content
                pattern = re.compile(r'<div class="col-lg-3 col-md-6 d-flex wow fadeInUp".*?</div>\s*</div>', re.DOTALL)
                ...
                for match in reversed(matches):
                    ...
                    # Extract image - handle both Jinja and static
                    img_match = re.search(r'src="({%.*?%}|{{.*?}}|.*?)"', block)
                    img_url = img_match.group(1) if img_match else ""
                    
                    # Extract title - handle both Jinja and static
                    # Look for h4 and capture everything inside
                    title_match = re.search(r'<h4.*?>(.*?)</h4>', block, re.DOTALL)
                    title = title_match.group(1).strip() if title_match else ""
                    
                    # Extract desc - handle both Jinja and static
                    # Look for p and capture everything inside
                ...
                # Update row class and container class
