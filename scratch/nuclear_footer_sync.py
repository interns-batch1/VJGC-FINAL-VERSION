...
def nuclear_footer_sync(directory):
    # Regex to find any footer-related block (HTML and Style)
    patterns_to_remove = [
        re.compile(r'<style>.*?footer-modern.*?</style>', re.DOTALL | re.IGNORECASE),
        re.compile(r'<div class="footer-mini">.*?</div>\s*<!--\s*Vijayalakshmi Group Site Utils\s*-->', re.DOTALL),
        re.compile(r'<div class="footer-mini">.*?</div>', re.DOTALL),
        re.compile(r'<div class="footer-modern">.*?</div>\s*<!--\s*/\.footer-modern\s*-->', re.DOTALL),
        re.compile(r'<footer class="footer-modern">.*?</footer>', re.DOTALL),
        re.compile(r'<footer class="footer-vjs-premium">.*?</footer>', re.DOTALL),
        ...
        for file in files:
            if file.endswith('.html') and file != 'index-2.html.bak':
                ...
                if wrapper_match:
                    ...
                    print(f"Nuclear sync success in {file}")
                else:
                    ...
                    if '</body>' in new_content:
                        ...
                        with open(file_path, 'w', encoding='utf-8') as f:
                            f.write(final_content)
                        print(f"Nuclear fallback in {file}")
                    else:
                        print(f"CRITICAL FAILURE in {file}")

if __name__ == "__main__":
    templates_dir = r'c:\Users\Admin\vjgc-final\vjs-website\templates'
    nuclear_footer_sync(templates_dir)
