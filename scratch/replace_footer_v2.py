...
for filename in os.listdir(TEMPLATE_DIR):
    if filename.endswith('.html'):
        ...
        # Pattern 1: Any block containing footer-container and Vijayalakshmi
        # This is very broad but usually effective for footers
        patterns = [
            r'<(div|footer)[^>]*class="footer-(modern|mini)"[^>]*>.*?</\1>(\s*<!-- /.footer-(modern|mini) -->)?',
            r'<footer>.*?</footer>',
            r'<(div|footer)[^>]*>.*?footer-container.*?Vijayalakshmi.*?</\1>'
        ]

        replaced = False
        for pattern in patterns:
            if re.search(pattern, content, re.DOTALL | re.IGNORECASE):
                # Ensure it's not replacing the whole body or something
                match = re.search(pattern, content, re.DOTALL | re.IGNORECASE).group(0)
                if len(match) < 10000: # Footers are usually not THAT huge
                    content = re.sub(pattern, NEW_FOOTER, content, flags=re.DOTALL | re.IGNORECASE)
                    replaced = True
                    break
        
        # Pattern 2: Special case for index-2 and others where the tag might be missing but container is there
        if not replaced and 'footer-container' in content:
             # Find the block starting with <div class="footer-container"> and ending before scripts
             content = re.sub(r'<div class="footer-container">.*?</footer>', NEW_FOOTER, content, flags=re.DOTALL | re.IGNORECASE)
             # And also handle if it's just a div
             content = re.sub(r'<div class="footer-container">.*?</div>\s*</div>\s*<!-- Optional JavaScript -->', NEW_FOOTER, content, flags=re.DOTALL | re.IGNORECASE)

        # Remove scroll button and script
        content = re.sub(r'<button id="vjs-scroll-top".*?</script>', '', content, flags=re.DOTALL)
        
        # Cleanup double footer comments
